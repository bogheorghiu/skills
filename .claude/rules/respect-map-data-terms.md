...
- Scripted requests to these services send a User-Agent that names the application, go one at a time, and stay within each policy's rate limit. A relay is acceptable only if it forwards that User-Agent. The services run on donated capacity, and the Nominatim policy says unacceptable use "will get you banned"; a ban hits every user of the skill.
