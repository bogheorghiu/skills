...
Each suite runs standalone from the repo root (`python3 <path>`), exits non-zero on failure, and is listed by name in `.github/workflows/skills-ci.yml`. A suite not listed there never runs in CI, and a suite that never runs looks exactly like one that passes.
