---
name: trip-scout
description: Find cheap flights and/or lodging (Ryanair, Wizz, Airbnb, Booking, hotels, B&Bs) for a dated personal trip via a scraping connector, telling the user the legal risks before any paid run, and deliver a visual comparison page. Self-maintains its list of working data sources.
...
**Load order.** Read this file. Resolve the user overlay without asking (`patterns/adapt.md` §Overlay). Then read the patterns the task needs and the sources, taking an overlay file over the bundled one with the same name.
...
2. **Sources.** Run `python3 <this skill's folder>/scripts/sources.py check` if Python is available; an entry it rejects counts as `untested` this session. Resolve which actors and sites to use from `sources/` plus the overlay (entry format: `sources/README.md`). Prefer entries with `status: working` and a recent `last_verified`.
...
5. **Search.** Run `patterns/flights.md` and/or `patterns/lodging.md`. Read each actor's input schema before its first call. Set each run's spend cap from the budget left; if the connector has no cap field, or refuses a cap as too low, say so and ask before the run.
...
9. **Close.** Say what wasn't checked, remind the user that the connector keeps the raw run data (host fields included) in their account until they delete it, name the assumption most worth pressing on, and give one concrete next step.
