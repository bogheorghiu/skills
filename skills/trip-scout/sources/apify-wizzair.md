...
caveats: Cheapest fare per day only: no flight times, no bags. Currency follows the route (outbound RON, inbound EUR on OTP⇄BLQ); convert per patterns/flights.md step 2, never add the raw amounts.
drop: —
