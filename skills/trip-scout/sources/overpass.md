...
---
how: At most two queries per trip: one small `node[entrance](around:300,LAT,LON)` query for the venue anchor (patterns/brief.md), and one for the map's base layer: streets, railways and the venue footprint inside a small bounding box, `[out:json][timeout:25];(way[highway](S,W,N,E);way[railway=rail](S,W,N,E););out geom;`. Simplify the geometry before shipping it in `streets.js`. Send a User-Agent naming the application; no parallel queries.
