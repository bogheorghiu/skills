...
- **Spend.** A total budget for the scraping runs, in the connector's currency (suggest USD 8–10 for flights and lodging if the user has no number; some actors refuse a cap under about USD 0.60). A lodging search alone is often 5–8 paid runs, so each run gets its share of what is left.
...
- **The anchor is the venue's visitor entrance, not the venue.** Every distance, isochrone and "closest option" hangs on this one point, so settle it before searching lodging:
  ...
  - Geocode that entrance in OpenStreetMap: an `entrance=*` node or gate if one exists (a small Overpass `node[entrance](around:300,LAT,LON)` query finds unnamed ones; the geocoder only finds named places); otherwise the pedestrian forecourt it is named after (`highway=pedestrian` area). Not a same-named road, a car park, a bus stop or the grounds' centroid, which can sit 100+ m away. Record the OSM type/id.
